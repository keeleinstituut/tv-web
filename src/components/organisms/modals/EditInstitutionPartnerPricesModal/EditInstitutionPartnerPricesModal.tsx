...
import {
  ...
  useParallelUpdateInstitutionPartnerPrices,
} from 'hooks/requests/useInstitutionPartners'
import { useFetchSkills, useSkillsCache } from 'hooks/requests/useVendors'
...
import { PayloadItem } from 'types/vendors'
...
export interface EditInstitutionPartnerPricesModalProps {
...
const EditInstitutionPartnerPricesModal: FC<EditInstitutionPartnerPricesModalProps> =
  ({ languageDirectionKey, skillId, institution_partner_id, filters, ...rest }) => {
    const { t } = useTranslation()
    const newLanguagePair = languageDirectionKey === 'new'
    const [src, dst] = split(languageDirectionKey, '_')

    const { parallelUpdating } = useParallelUpdateInstitutionPartnerPrices({
    ...
    const { prices: pricesData } = useAllInstitutionPartnerPricesFetch({
      initialFilters: {
        institution_partner_id,
        lang_pair: [{ src, dst }],
        per_page: 50,
    ...
    }, [currentLanguagePrices, dst, languageDirectionKey, newLanguagePair, skills, src])
    ...
    const onEditPricesSubmit: SubmitHandler<FormValues> = useCallback(
      async (values) => {
        ...
        const updateSkills = isEmpty(updateAbleSkills)
          ? null
          : {
              prices: getNewStateData(updateAbleSkills),
              state: DataStateTypes.UPDATED,
            }

        const updatePricesPayload: { data: PayloadItem[] } = {
              ...
              if (errors) {
                map(
                  ...
                  (errorsArray: unknown[], key) => {
                    ...
                    } else if (fieldName !== 'institution_partner_id') {
                      setError(
                        `${languageDirectionKey}.priceObject.${
                          erroredPrices?.[toNumber(arrayIndex)]?.skill_id
                        }.${fieldName}`,
...
export default EditInstitutionPartnerPricesModal
