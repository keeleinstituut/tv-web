...
import { useFetchMTJobHistory } from 'hooks/requests/useMachineTranslation'
  ...
  const tableData: JobRow[] = useMemo(
    () =>
      jobs.map((job: TranslationJob) => ({
        ...
        status: t(
          `machine_translation.file_status_${job.status}` as Parameters<
            typeof t
          >[0]
        ),
    ...
    () =>
      [
        ...
        columnHelper.accessor('languages', {
          ...
          cell: ({ getValue }) => (
            <span className={classes.languages}>{getValue()}</span>
          ),
          size: 120,
        }),
