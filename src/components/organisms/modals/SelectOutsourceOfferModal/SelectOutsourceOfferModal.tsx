...
export interface SelectOutsourceOfferModalProps {
  isModalOpen?: boolean
  closeModal: () => void
  requestId: string
}
...
const isInPlay = (status: OutsourceOfferStatus) =>
  status === OutsourceOfferStatus.RequestAccepted ||
...
const SelectOutsourceOfferModal: FC<SelectOutsourceOfferModalProps> = ({
  isModalOpen,
  closeModal,
  requestId,
}) => {
  ...
  const loserOffers = useMemo(
    () =>
      offers.filter((o) => o.id !== winnerId && isInPlay(o.status)),
    [offers, winnerId]
  )
      ...
      {isLoading || !request ? (
        ...
        <div className={classes.recipientList}>
          {offers.map((offer) => {
            ...
            const showRejection = !!winnerId && !isWinner && isInPlay(offer.status)
...
export default SelectOutsourceOfferModal
